...
void Commands::processPacket(QByteArray data)
{
    ...
    case COMM_GET_VALUES_SELECTIVE: {
        ...
        if (mask & (uint32_t(1) << 10)) { //total trip time
            values.amp_hours_charged = vb.vbPopFrontDouble32(1e0);
        }
        if (mask & (uint32_t(1) << 11)) {
            values.watt_hours = vb.vbPopFrontDouble32(1e4);
        }
        if (mask & (uint32_t(1) << 12)) {//6v analog 3
            values.watt_hours_charged = vb.vbPopFrontDouble32(1e2);
        }
        if (mask & (uint32_t(1) << 13)) { //Throttle voltage
            values.tachometer =  vb.vbPopFrontDouble32(1e0);
        }
        if (mask & (uint32_t(1) << 14)) { //Regen voltage
            values.tachometer_abs =  vb.vbPopFrontDouble32(1e0);
        }
        ...
        if (vb.size() >= 4) {
            if (mask & (uint32_t(1) << 16)) {//digital inputs
                values.position = vb.vbPopFrontUint32();
            }
        } else {
            values.position = -1.0;
        }
...
QString Commands::faultToStr(mc_fault_code fault)
{
    ...
    case FAULT_CODE_HALL_SENSOR:return"FAULT_CODE_HALL_SENSOR";
    case FAULT_CODE_THROTTLE_VOLTAGE:return"FAULT_CODE_THROTTLE_VOLTAGE";
         case FAULT_CODE_REGEN_VOLTAGE:return"FAULT_CODE_REGEN_VOLTAGE";
    }
